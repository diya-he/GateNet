...
def parse_args() -> argparse.Namespace:
    ap = argparse.ArgumentParser(
        description="Train gate-lite YOLO26-seg with 192x192 mask prototypes by default."
    )
    ap.add_argument("--data", type=Path, default=Path("data/ultralytics/image1_monorace_aug_yolo26/dataset.yaml"))
    ap.add_argument(
        "--model",
        type=Path,
        default=Path("ultralytics/ultralytics/cfg/models/26/yolo26-gate-lite-himask-seg.yaml"),
    )
    ap.add_argument(
        "--init-weights",
        type=Path,
        default=Path("runs/segment/runs/yolo26/image1_monorace_aug_yolo26_gate_lite_himask_seg_384/weights/best.pt"),
    )
    ...
    ap.add_argument("--name", type=str, default="image1_monorace_aug_yolo26_gate_lite_himask_seg_384")
    ap.add_argument("--seed", type=int, default=42)
    ap.add_argument("--mask-ratio", type=int, default=1)
    ap.add_argument("--overlap-mask", action="store_true", help="Use merged overlap-mask supervision.")
    ap.add_argument("--mosaic", type=float, default=0.8)
    ap.add_argument("--close-mosaic", type=int, default=15)
    ap.add_argument("--mixup", type=float, default=0.08)
    ap.add_argument("--cutmix", type=float, default=0.0)
    ap.add_argument("--copy-paste", type=float, default=0.25)
    ap.add_argument("--copy-paste-mode", choices=("flip", "mixup"), default="mixup")
    ap.add_argument("--degrees", type=float, default=12.0)
    ...
    ap.add_argument("--hsv-s", type=float, default=0.65)
...
def train(args: argparse.Namespace, run_dir: Path) -> Path:
    ...
    if args.init_weights and args.init_weights.exists():
        print(f"Loading partial init weights: {args.init_weights}")
        model.load(str(args.init_weights))
    elif args.init_weights:
        print(f"Init weights not found, training from YAML: {args.init_weights}")
    ...
    model.train(
        ...
        copy_paste_mode=str(args.copy_paste_mode),
...
def main() -> None:
    ...
    onnx_path = None if args.no_export else export_topk(save_dir, args)
