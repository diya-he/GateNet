...
## GateNet local YOLO26 modifications

This checkout is used by the GateNet project as a local Ultralytics fork
(`ultralytics.__version__ == 8.4.90`). In addition to the upstream YOLO26 models, it
adds gate-focused lightweight segmentation configs and a high-resolution prototype head:

- `ultralytics/cfg/models/26/yolo26-gate-lite-seg.yaml` keeps the YOLO26 `Segment26`
  segmentation head, but reduces the model scale to `depth=0.50`, `width=0.125`, and
  `max_channels=512`. It also uses fewer mask coefficients/prototype channels
  (`nm=16`, `npr=128`) than stock YOLO26n-seg (`nm=32`, `npr=256`).
- `ultralytics/cfg/models/26/yolo26-gate-lite-himask-seg.yaml` keeps the same lightweight
  backbone/head and switches to `Segment26HighRes` with `proto_scale=2`. With `384x384`
  input, the mask prototypes increase from stride-4 `96x96` to `192x192`.
- `Proto26HighRes` and `Segment26HighRes` are registered in `ultralytics/nn/modules/`
  and `ultralytics/nn/tasks.py`. The high-resolution branch uses interpolation plus a
  lightweight refinement layer, improving fine gate/ring mask boundaries without
  returning to a heavy full-width YOLO26 segmentation model.
- GateNet training scripts call this local package through `--ultralytics-root
  ultralytics`; `scripts/train_yolo26_gate_lite_himask_seg.py` also trains with
  `mask_ratio=1` to preserve detailed mask supervision.

The intended tradeoff is to make the detector and feature extractor lighter for speed,
while allocating more spatial resolution to the ProtoNet/mask branch where instance
segmentation quality is most sensitive.
