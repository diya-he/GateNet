...
class Proto26HighRes(Proto26):
    """YOLO26 mask Proto module with an extra lightweight upsample stage for finer ring masks."""

    def __init__(self, ch: tuple = (), c_: int = 256, c2: int = 32, nc: int = 80, scale: int = 2):
        """Initialize a high-resolution Proto26 head.

        The base Proto26 produces masks at input stride 4. With scale=2, the
        output becomes stride 2, e.g. 384x384 input -> 192x192 prototypes.
        """
        super().__init__(ch, c_, c2, nc)
        self.highres_scale = int(scale)
        self.highres_refine = Conv(c_, c_, k=3, g=c_) if self.highres_scale > 1 else nn.Identity()

    def forward(self, x: torch.Tensor, return_semantic: bool = True) -> torch.Tensor:
        """Fuse multi-scale features and generate higher-resolution mask prototypes."""
        feat = x[0]
        for i, f in enumerate(self.feat_refine):
            up_feat = f(x[i + 1])
            up_feat = F.interpolate(up_feat, size=feat.shape[2:], mode="nearest")
        ...
        y = self.feat_fuse(feat)
        y = self.cv2(self.upsample(self.cv1(y)))
        if self.highres_scale > 1:
            y = F.interpolate(y, scale_factor=self.highres_scale, mode="bilinear", align_corners=False)
            y = self.highres_refine(y)
        p = self.cv3(y)
