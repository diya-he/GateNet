...
from .block import DFL, SAVPE, BNContrastiveHead, ContrastiveHead, Proto, Proto26, Proto26HighRes, RealNVP, Residual, SwiGLUFFN
...
class Segment26HighRes(Segment26):
    """YOLO26 segmentation head with higher-resolution mask prototypes."""

    def __init__(
        self, nc: int = 80, nm: int = 32, npr: int = 256, proto_scale: int = 2, reg_max=16, end2end=False, ch: tuple = ()
    ):
        """Initialize Segment26HighRes.

        Args:
            nc (int): Number of classes.
            nm (int): Number of mask coefficients.
            npr (int): Intermediate proto channels.
            proto_scale (int): Extra proto upsample factor after the normal stride-4 prototype.
            reg_max (int): DFL bins.
            end2end (bool): Whether to use end-to-end detection.
            ch (tuple): Input feature channels.
        """
        super().__init__(nc, nm, npr, reg_max, end2end, ch)
        self.proto_scale = int(proto_scale)
        self.proto = Proto26HighRes(ch, self.npr, self.nm, nc, scale=self.proto_scale)
