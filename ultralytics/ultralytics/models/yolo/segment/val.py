...
class SegmentationValidator(DetectionValidator):
    ...
    def init_metrics(self, model: torch.nn.Module) -> None:
        ...
        head = model
        for _ in range(4):
            modules = getattr(head, "model", None)
            if isinstance(modules, (torch.nn.ModuleList, torch.nn.Sequential, list, tuple)):
                head = modules[-1]
                break
            if modules is None or modules is head:
                break
            head = modules
        proto_scale = int(getattr(head, "proto_scale", 1))
        self.proto_stride = max(1, 4 // proto_scale)
    ...
    def postprocess(self, preds: list[torch.Tensor]) -> list[dict[str, torch.Tensor]]:
        ...
        imgsz = [self.proto_stride * x for x in proto.shape[2:]]  # get image size from proto
    ...
    def _prepare_batch(self, si: int, batch: dict[str, Any]) -> dict[str, Any]:
        ...
        if nl:
            mask_size = [
                s if self.process is ops.process_mask_native else s // self.proto_stride for s in prepared_batch["imgsz"]
            ]
    ...
    def _process_batch(self, preds: dict[str, torch.Tensor], batch: dict[str, Any]) -> dict[str, np.ndarray]:
        ...
        if gt_cls.shape[0] == 0 or preds["cls"].shape[0] == 0:
            ...
            if gt_masks.shape[1:] != pred_masks.shape[1:]:
                gt_masks = F.interpolate(gt_masks[:, None].float(), pred_masks.shape[1:], mode="bilinear")[:, 0]
